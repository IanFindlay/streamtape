...
import twitch_recorder as tr


class TestCalcStart(unittest.TestCase):
    """Test for calc_start function."""
    ...
    def test_later_hour_yields_same_day_date_and_correct_time(self):
        ...
        now = dt.datetime.now()
        start_time = tr.calc_start('{}:00'.format(now.hour + 1))
    ...
    def test_start_time_same_hour_as_current_but_earlier_yields_next_day(self):
        ...
        now = dt.datetime.now()
        start_time = tr.calc_start('{}:{}'.format(now.hour,
                                                  now.minute - 1))
        self.assertEqual(now.day + 1, start_time.day)
